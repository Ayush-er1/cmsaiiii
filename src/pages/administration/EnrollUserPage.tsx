...
// Import types
import { AccountFormData, ProfileFormData, StudentFormData, roleLabels } from "@/types/users";

// Import new sub-components
import { StepOneAccount } from "@/components/features/users/enrollment/StepOneAccount";
import { StepTwoRoleDetails } from "@/components/features/users/enrollment/StepTwoRoleDetails";
...
export function EnrollUserPage() {
    ...
    const handleNextStep = () => {
        // Validate Step 1
        if (!accountData.firstName.trim()) return toast({ title: "First Name is required", variant: "destructive" });
        if (!accountData.lastName.trim()) return toast({ title: "Last Name is required", variant: "destructive" });
        if (!accountData.userId.trim()) return toast({ title: "User ID is required", variant: "destructive" });
        if (!accountData.email.trim()) return toast({ title: "Email is required", variant: "destructive" });
    ...
    const handleSave = async () => {
        ...
        if (!profileData.department && profileData.role !== 'super_admin') {
            return toast({ title: "Please select a Department", variant: "destructive" });
        }
        ...
        if (profileData.role === "student") {
            if (
                ...
                return toast({ title: "Please fill in all student details", variant: "destructive" });
            }
        }
        ...
        try {
            ...
            if (editingUserId) {
                ...
                toast({ title: `${roleLabels[profileData.role]} enrolled successfully` });
            }
            setLocation("/users");
        } catch (err: any) {
